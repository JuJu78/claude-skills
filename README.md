...
|---|---|---|
| **ai-mode-tracker** | Track how a brand or person appears in Google AI Mode and Google AI Overviews. N repetitions per prompt (these engines are non-deterministic), deterministic extraction of cited entities, inline citations and retrieved sources, inline-citation vs retrieved-source gap analysis — everything appended to a Google Sheet with no Google API. Bilingual FR/EN triggers. | [JuJu78/google-ai-visibility-tracker](https://github.com/JuJu78/google-ai-visibility-tracker) |
...
| **gsc-micro-content-optimizer** | Turn Google Search Console query data into surgical on-page micro-edits without rewriting the page. Classifies each visible query (exact match, close variant, missing), scores candidates, and proposes before/after snippets. Audit-only by default, CSV-compatible, no API dependency required. | [JuJu78/gsc-micro-content-optimizer](https://github.com/JuJu78/gsc-micro-content-optimizer) |
